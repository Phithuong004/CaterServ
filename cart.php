...
<html lang="en">
    ...
    <head>
        ...
        <style>
            ...
            .cart-item input[type="number"] {
                width: 50px;
                margin: 0 10px;
                text-align: center; /* Center the quantity input */
            }
            .cart-item button {
                text-align: center;
            ...
            .total-price {
                border-top: 1px solid #000; /* Add a horizontal line */
                padding-top: 20px;
                margin-top: 20px;
                font-size: 24px;
                color: black;
    ...
    <body>
        ...
        <?php
include_once 'db_connect.php'; // Đảm bảo rằng bạn đã bao gồm file db_connect.php

// Kiểm tra xem người dùng đã đăng nhập hay chưa
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin']) {
    // Lấy ID người dùng từ $_SESSION hoặc bất kỳ nguồn dữ liệu nào khác
    $userId = $_SESSION['userid'];

    // Truy vấn để lấy thông tin sản phẩm từ bảng "cart"
    $sql = "SELECT cart.food_id, cart.quantity, cart.name, cart.price, cart.chitiet, cart.image FROM cart WHERE cart.user_id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        // Hiển thị thông tin về sản phẩm trong giỏ hàng và biểu mẫu
        while ($row = $result->fetch_assoc()) {
            $totalPrice += $row['price'] * $row['quantity']; // Tính tổng tiền
            echo "<div class='cart-item'>";
            echo "<img src='{$row['image']}' alt='{$row['name']}' />";
            echo "<div class='details'>";
            echo "<p>{$row['name']}</p>";
            echo "<p>Chi tiết: {$row['chitiet']}</p>";
        ...
        echo "<p class='total-price'>Tổng tiền: " . number_format($totalPrice, 0, ',', '.') . " VNĐ</p>"; // Hiển thị tổng tiền
        echo '<a href="checkout.php" class="buttonend btn btn-primary">Checkout</a>'; // Nút thanh toán
    } else {
        echo "<p style='text-align: center; color: red; animation: blinker 1s linear infinite;'>Không có sản phẩm nào trong giỏ hàng của bạn.</p>";
    }
} else {
    echo '<p>Vui lòng <a href="login.php">đăng nhập</a> hoặc <a href="signup.php">đăng ký</a> để tiếp tục.</p>';
}

// Xử lý yêu cầu POST để cập nhật hoặc xóa sản phẩm
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Kiểm tra xem yêu cầu có chứa food_id không
    if (isset($_POST['food_id'])) {
        $food_id = $_POST['food_id'];
        
        // Nếu người dùng chọn nút giảm số lượng
        if (isset($_POST['decrease'])) {
            // Kiểm tra số lượng sản phẩm hiện tại
            $sql = "SELECT quantity FROM cart WHERE food_id = ? AND user_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ii", $food_id, $_SESSION['userid']);
            $stmt->execute();
            $result = $stmt->get_result();
            if ($result->num_rows > 0) {
                ...
                if ($currentQuantity > 1) {
                    ...
                    $sql_update = "UPDATE cart SET quantity = ? WHERE food_id = ? AND user_id = ?";
                    $stmt_update = $conn->prepare($sql_update);
                    $stmt_update->bind_param("iii", $newQuantity, $food_id, $_SESSION['userid']);
                    $stmt_update->execute();
                }
            }
        }
        
        // Nếu người dùng chọn nút tăng số lượng
        if (isset($_POST['increase'])) {
            // Tăng số lượng sản phẩm
            $sql_update = "UPDATE cart SET quantity = quantity + 1 WHERE food_id = ? AND user_id = ?";
            $stmt_update = $conn->prepare($sql_update);
            $stmt_update->bind_param("ii", $food_id, $_SESSION['userid']);
            $stmt_update->execute();
        }
        
        // Nếu người dùng chọn nút xóa
        if (isset($_POST['delete'])) {
            // Xóa sản phẩm khỏi giỏ hàng
            $sql_delete = "DELETE FROM cart WHERE food_id = ? AND user_id = ?";
            $stmt_delete = $conn->prepare($sql_delete);
            $stmt_delete->bind_param("ii", $food_id, $_SESSION['userid']);
            $stmt_delete->execute();
        }
    }
}
